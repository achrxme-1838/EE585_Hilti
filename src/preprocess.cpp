...
void Preprocess::hesai_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) 
{
    ...
    // Because the time, i.e. the value in the `curvature` is the relative time w.r.t. the time of starting point.
    double smallest_time = DBL_MAX;
    bool is_not_updated = true;
    for (const auto& pt: pl_orig.points) {
        if (pt.timestamp < smallest_time) {
            smallest_time = pt.timestamp;
            is_not_updated = false;
        }
    }
    if (is_not_updated) {
        throw std::invalid_argument( "Something's wrong. The samellest time is not updated" );
    }
    ...
    for (int i = 0; i < pl_orig.points.size(); i++) {
        ...
        // curvature unit: ms and it is the relative time!!
